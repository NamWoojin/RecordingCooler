...
public class RecordingCoolerInfoHandler implements RequestHandler<Document, String> {
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        // temperature와 motor_step 값이 이전상태와 동일한 경우 테이블에 저장하지 않고 종료 
        if (document.current.state.reported.temperature.equals(document.previous.state.reported.temperature) && 
                document.current.state.reported.motor_step.equals(document.previous.state.reported.motor_step) /*&&
                document.current.state.reported.user_mode.equals(document.previous.state.reported.user_mode)*/) {
                return null;
        }
        ...
        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item().withPrimaryKey("deviceId", document.device)
                        ...
                        .withString("motor_step", document.current.state.reported.motor_step)
                        .withString("user_mode", document.current.state.reported.user_mode)
